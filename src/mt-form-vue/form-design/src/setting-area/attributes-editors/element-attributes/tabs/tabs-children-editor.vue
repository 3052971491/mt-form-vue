<template>
  <a-form-item :label="t('currentTab')" name="children">
    <a-row
    ...
    <a-row>
      <a-button @click="handleAddClick">
        <plus-outlined />{{ t("addTabs") }}</a-button
      >
    </a-row>
  </a-form-item>
</template>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function handleAddClick() {
      const newEl = central.value.buildContainerElement("tab-pane");
    ...
    function handleDelClick(index: number) {
      if (el.value.children.length <= 1) {
        message.warning(t("deleteTabErrorMsg"));
        return;
      }
